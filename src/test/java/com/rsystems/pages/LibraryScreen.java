...
public class LibraryScreen extends TestInitization {
	...
	public  boolean VerifyMenuItemsTextOrFonts(List<String> actualList,String keyName) throws InterruptedException
	{
		...
		{	
			...
			reports.log(LogStatus.PASS,"Expected List" + expectedList.get(i) + "Actual List :"+libraryMenuItems().get(i).getText());
	 ...
	 *  Created By Rahul Dhoundiyal
	 */
	public  boolean verifyLibrarySubMenuTextOrFonts(List<String> actualList,String keyName) throws InterruptedException{
		...
		for(int i=0;i<libraryMenuItems().size();i++)
		{
			...
			{	
				...
				{
					expectedList.add(TestInitization.getExcelKeyValue("library", libraryMenuSubItems(i).get(j).getAttribute("innerText"), keyName));
				}
			}
		}
	...
	public String fetchTwoLinesLevel()
	{
		...
		if (upCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, -60px, 0px)") && downCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, 70px, 0px)"))
		{
			level =  "Level2";
		}
		else if (upCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, -70px, 0px)") && downCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, 70px, 0px)"))
		{
			level =  "Level1";
		}
		else if (upCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, -120px, 0px)") && downCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, 288px, 0px)"))
		{
			level =  "Level3";
		}
		...
		for(int i = 0 ; i< libraryMenuItems().size(); i++)
		{
			if(!libraryMenuItems().get(i).getText().equalsIgnoreCase("mijn apps"))
			{
				...
				for(int  j =0; j< 5 ;j++)
				{
					sendKeyMultipleTimes("DOWN", 1, 1000);
					driver.switchTo().defaultContent();
					if (upCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, -120px, 0px)") && downCanvasLine.getAttribute("style").split(";")[1].trim().contains("translate3d(0px, 288px, 0px)"))
					{
