...
public class Hub extends TestInitization {
	...
	public List<WebElement> hubMenuItems() {
		 ...
		 * including their translation, font-size, position etc. Created by
		 * Nitin Kaushik
		 */
	...
	public List<String> getHubMenuItemsText() {
		...
		System.out.println(
				"Inside hubMenuItemsTranslation method ::: This function will return the list of Hub Menu Items Text");
		...
		for (int i = 0; i < driver.findElements(By.xpath("//li[contains(@id,'menuItem')]")).size(); i++) {
			...
			if (i < 3) {
				...
				System.out.println("Adding value in the array list at position + " + i + " "
						+ driver.findElements(By.xpath("//li[contains(@id,'menuItem')]")).get(i).getText());

			} else {
				String imageURL = driver.findElements(By.xpath("//li[contains(@id,'menuItem')]")).get(i)
						.getCssValue("background-image");
				hubMenuItemList.add(imageURL.split("/")[imageURL.split("/").length - 1].replace("\")", ""));
				System.out.println("Adding value in the array list at position + " + i + " "
						+ imageURL.split("/")[imageURL.split("/").length - 1].replace("\")", ""));
			}
	...
	public List<String> getHubMenuItemsFonts() {
		...
		System.out.println(
				"Inside getHubMenuItemsFonts method ::: This function will return the list of Hub Menu Items Fonts");
	...
	public String returnElementTextOrImageAtGivenLocation(WebElement element, String elementExcelX,
			String elementExcelY) {
		...
		System.out.println(
				"Element returned x & y :: " + element.getLocation().getX() + " ::::: " + element.getLocation().getY());
		...
		if (element.getLocation().getX() == x && element.getLocation().getY() == y) {
			...
			if (element.getText() != null) {
				...
				System.out.println(
						"Elemnt's Image returned from X and Y cordinates : " + element.getCssValue("background-image"));
				log.info(
						"Elemnt's Image returned from X and Y cordinates : " + element.getCssValue("background-image"));
	...
	/**
	 * This test cases is used to verify navigation in right side of Text Line
	 * in Hub Screen. Created By Rahul
	 */
		...
		for (int i = 1; i < hubMenuItems().size(); i++) {
			...
			if (actualElement.equalsIgnoreCase(ObjectRepository.HubMenuItemsNLFocused[i])) {
				reports.log(LogStatus.PASS,
						"SEND RIGHT Key : Expected Output - Focus should be on -"
								+ ObjectRepository.HubMenuItemsFocused[i] + ". Actual Output - Focus is on "
								+ actualElement.split("_")[0]);
				reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
			} else {
				FailTestCase("Test cases is failed as Expected Output - Focus should be on -"
						+ ObjectRepository.HubMenuItemsNLFocused[i] + ". Actual Output - Focus is on " + actualElement);
			}
		}
		sendKeyMultipleTimes("RIGHT", 1, 1000);
		if (actualElement.equalsIgnoreCase(
				ObjectRepository.HubMenuItemsNLFocused[ObjectRepository.HubMenuItemsNLFocused.length - 1])) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT Key Again: Expected Output - Focus should be on -:"
							+ ObjectRepository.HubMenuItemsFocused[ObjectRepository.HubMenuItemsFocused.length - 1]
							+ ". Actual Output - Focus is on " + actualElement.split("_")[0] + "Nothing happened");
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase("Test case is failed as  Expected Output - Focus should be on -:"
					+ ObjectRepository.HubMenuItemsNLFocused[ObjectRepository.HubMenuItemsNLFocused.length - 1]
					+ "Focus is on " + actualElement + "Nothing happened");
		}

	}

	/**
	 * This test cases is used to verify navigation in left side of Text Line in
	 * Hub Screen. Created By Rahul
	 */
	private void verifyHubLeftSideNavigation() throws InterruptedException {
		...
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Library", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND LEFT Key : Expected Output - Focus should be on Library. Actual Output- Focus is on "
							+ focusHubElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
					"Test Cases is failed as Expected Output - Focus should be on Library. Actual Output- Focus is on "
							+ focusHubElement.getText());
		}
		sendKeyMultipleTimes("LEFT", 1, 1000);
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Library", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND LEFT Key Again : Expected Output - Focus should be on Library. Actual Output- Focus is on "
							+ focusHubElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
	...
	/**
	 * This test cases is used to verify navigation in right side of Asset Line
	 * in Hub Screen. Created By Rahul
	 */
	private void verifyHubAssetLineRightSideNavigation() throws InterruptedException {
		...
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "LiveTV", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT KEY :- Expected Output - Focus should be on televisie. Actual Output - Focus is on "
							+ focusHubElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
					"Test case is failed as Expected Output - Focus should be on televisie. Actual Output - Focus is on "
							+ focusHubElement.getText());
		}
		sendKeyMultipleTimes("RIGHT", 1, 1000);
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Shop", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT KEY :- Expected Output - Focus should be on Shop. Actual Output - Focus is on "
							+ focusHubElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
					"Test cases is failed as Expected Output - Focus should be on Shop. Actual Output - Focus is on "
							+ focusHubElement.getText());
		}
		...
		if (hubSearchTextElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Search", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT Key : Expected Output - Focus should be on Search Icon. Actual Output - Focus is on Search Icon");
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
					"Test Case is failed as Expected Output - Focus should be on Search Icon. Actual Output - Focus is not on Search Icon");
		}
		sendKeyMultipleTimes("RIGHT", 1, 1000);
		if (hubSettingTextElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Setting", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT Key : Expected Output - Focus should be on Settings Icon. Actual Output - Focus is on Settings Icon");
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase(
					"Test Case is failed as Expected Output - Focus should be on Settings Icon. Actual Output - Focus is not on Settings Icon");
		}
	}

	/**
	 * This test cases is used to verify navigation in left side of Asset Line
	 * in Hub Screen. Created By Rahul
	 */
	private void verifyHubAssetLineLeftSideNavigation() throws InterruptedException {
		...
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Library", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND LEFT Key : Focus should be on Library Assets. Actual Output:- Focus is on "
							+ focusHubElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase("Test cases is failed as Focus should be on Library Assets. Actual Output:- Focus is on "
					+ focusHubElement.getText());
		}
		...
		for (int i = 0; i < libraryItmes.size() - 1; i++) {
			...
			if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Library", "name_nl"))) {
				...
				FailTestCase(
						"Test cases is failed as Expected Output - Focus should be on Library Assets. Actual Output:- Focus is on "
								+ focusHubElement.getText());
			}
		}
		...
		for (int i = 0; i < libraryItmes.size() - 1; i++) {
			...
			if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Library", "name_nl"))) {
				reports.log(LogStatus.PASS,
						"SEND RIGHT Key : Expected Output - Focus should be on Library Assets. Actual Output:- Focus is on "
								+ focusHubElement.getText());
				reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
			} else {
				FailTestCase(
	...
	/**
	 * This test cases is used to verify Asset Line in Hub Screen above Text
	 * line Created By Rahul
	 */
	public void verifyAssetLine() throws InterruptedException {
		...
		if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "LiveTv", "name_nl"))) {
			...
			FailTestCase("Expected Output - Focus should be on TV Assets.  Actual Output - Focus is on "
					+ focusHubElement.getText());
		}
		for (int i = 0; i < shopItems.size(); i++) {
			...
			if (focusHubElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Shop", "name_nl"))) {
				...
				FailTestCase(
						"Test cases is failed as Expected Output - Focus should be on Shop Assets.  Actual Output - Focus is on "
								+ focusHubElement.getText());
			}
		}
		...
		if (hubSearchTextElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Search", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT Key :- Expected Output - Focus should be on :- "
							+ getExcelKeyValue("screenTitles", "Search", "name_nl") + " Actual Output - Focus is on "
							+ hubSearchTextElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase("Test cases is failed as Expected Output - Focus should be on :- "
					+ getExcelKeyValue("screenTitles", "Search", "name_nl") + " Actual Output - Focus is on "
		...
		if (hubSettingTextElement.getText().equalsIgnoreCase(getExcelKeyValue("screenTitles", "Setting", "name_nl"))) {
			reports.log(LogStatus.PASS,
					"SEND RIGHT Key :- Expected Output - Focus should be on :- "
							+ getExcelKeyValue("screenTitles", "Setting", "name_nl") + " Focus is on "
							+ hubSettingTextElement.getText());
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase("Test cases is failed as Expected Output - Focus should be on :- "
					+ getExcelKeyValue("screenTitles", "Setting", "name_nl") + " Focus is on "
	...
	public void verifyFocousLineMoveUp() throws InterruptedException {
		...
		if (menuItemContainer.getAttribute("class").trim().contentEquals("cMenuList")) {
			...
			FailTestCase("Focous is not move to asset line");
		}
	}
	...
	public void verifyFocousLineMoveDown() throws InterruptedException {
		...
		if (menuItemContainer.getAttribute("class").trim().contentEquals("cMenuList colorWhite")) {
			...
			FailTestCase("Focous is not move to text line");
		}
	}
	...
	public void hub_Text_line() throws InterruptedException {
		...
		if (menuLibraryCoordinate == tvGuideFilterCordinate && storeFilterCoordinate == searchCoordinate
				&& searchCoordinate == settingCoordinate) {
			reports.log(LogStatus.PASS,
					"mijn bibliotheek corordinate " + menuLibraryCoordinate + " televisie corordinate "
							+ tvGuideFilterCordinate + " shop corordinate " + storeFilterCoordinate
							+ " Search cordinate " + searchCoordinate + " Settings co-ordinate " + settingCoordinate
	...
	public void verifyFocousElementText(String expectedFocousElementText) throws InterruptedException {
		...
		if (focusHubElement.getText().trim().contentEquals(expectedFocousElementText)) {
			...
			FailTestCase("Unable to focous expected element Actual focous Element: " + focusHubElement.getText()
					+ " Expected focous element " + expectedFocousElementText);
		}
