...
public class RecordingScreen extends TestInitization {
	...
	/**
	 * This function is used to get Channel Number. Created By Rahul Dhoundiyal
	 */
	...
	/**
	 * This function is used to get Channel Info Image. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to get Episode Duration. Created By Rahul
	 * DHoundiyal
	 */
	...
	/**
	 * This function is used to get Channel Definition. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to start recording on future channel.It will return
	 * the episode details like channel number,program name,Info Image,Channel
	 * Definition Created By Rahul Dhoundiyal
	 */
	public List<EpisodeInfo> scheduleRecordingForFutureChannel(String recordingType, int numberOfRecording)
		...
		while (!stopRecording) {
			if (numberOfRecording != noOfRecordedChannel) {
				...
				if (!driver.findElement(By.xpath("//p[@id='headerTitle']")).getText()
				...
				if (recordingType.equalsIgnoreCase("SINGLE")) {
					...
					if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
						programDetails.add(new EpisodeInfo(getChannelNo(), epgepisodeName, getEpisodeDuration(),
								getChannelDefiniton()));
						...
						if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
							...
							reports.log(LogStatus.PASS, " Already recording is scheduled on  " + epgepisodeName
									+ " epsiode. Unable to record this episode");
							reports.attachScreenshot(captureCurrentScreenshot());
						...
						reports.log(LogStatus.PASS, " Already recording is scheduled on  " + getInfoEpisodeName()
								+ " epsiode. Unable to record this episode");
						reports.attachScreenshot(captureCurrentScreenshot());
						sendKeyMultipleTimes("PAGE_DOWN", 1, 2000);
						driver.switchTo().defaultContent();
		...
		while (!stopRecording) {
			...
			if (focusProgram
				...
				if (recordingType.equalsIgnoreCase("SINGLE")) {
					TestInitization.sendKeyMultipleTimes("DOWN", 1, 1000);
					if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
						episodeDetails = new EpisodeInfo(getChannelNo(), epgEpisodeName, getEpisodeDuration(),
								getChannelDefiniton());
						...
						if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
							...
							reports.log(LogStatus.PASS,
									"Already Recording is scheduled for current episode.Navigate to Future Epsiode");
					...
					if (activeInfoMenuItem.getText().equalsIgnoreCase("serie opnemen")) {
						...
						reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
						driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
						System.out.println(episodeDetails.toString() + episodeDetails.channelNo
								+ episodeDetails.programDefiniton + episodeDetails.programDuration);
						stopRecording = true;
					} else {
						TestInitization.sendKeyMultipleTimes("PAGE_DOWN", 1, 2000);
						TestInitization.sendKeyMultipleTimes("DOWN", 1, 2000);
	...
	/**
	 * This function is used to delete recording from scheduled recorded list
	 * under planning option of Library Created By Rahul Dhoundiyal
	 */
	public void deleteSchduledRecording(EpisodeInfo episodeDetails, String recordingType) throws InterruptedException {
		...
		for (int i = 0; i < recordingContentElementList.size(); i++) {
			if ((recordingContentElementList.get(i).getAttribute("assetvolume").equalsIgnoreCase(recordingType))) {
				if (recordingContentElementList.get(i)
						...
						&& recordingContentElementList.get(i)
								.findElement(By
						...
						&& recordingContentElementList.get(i)
								.findElement(By.cssSelector(
										ObjectRepository.RecordingElements.ProgramDurationInPlannedRecording))
								.getText().split("-")[1].trim().equalsIgnoreCase(episodeDetails.programDuration)) {
		...
		TestInitization.sendKeyMultipleTimes("ENTER", 1, 1000);
		reports.log(LogStatus.PASS,
				"Expected Output - " + episodeDetails.programName
						+ " Info Screen should be displayed with Delete Option " + " Actual Output - "
						+ episodeDetails.programName + " Info Screen getting displayed with Delete Option ");
		reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		TestInitization.sendKeyMultipleTimes("ENTER", 1, 1000);
	...
	/**
	 * This function is used to verify the recording is scheduled properly or
	 * not. Created By Rahul Dhoundiyal
	 * 
	 */
	public boolean verifyRecordingIsScheduledOrNot(EpisodeInfo episodeDetails, String recordingType)
			throws InterruptedException {
	 ...
	 * 
	 */
	public boolean verifyRecordingisDeletedOrNot(EpisodeInfo episodeDetails, String recordingType)
			throws InterruptedException {
	...
	/**
	 * This function is used to verify navigation under recordings under planned
	 * recording
	 * 
	 * @return Created By Rahul Dhoundiyal
	 */
	public boolean verifyNavigationInPlannedRecording() throws InterruptedException {
		...
		/*
		 * If there is no recording scheduled the first start multiple recording
	...
	/**
	 * This function is used to delete all Single Recordings from Planned
	 * recordings Created By Rahul Dhoundiyal
	 */
	public void deletePlannedSingleRecording() throws InterruptedException {
		...
		if (singleRecordingDelete) {
			...
			if (totalRecordings != Integer.parseInt(totalRecordingID.getText())) {
				reports.log(LogStatus.PASS,
						"Recording is deleted successfully and List getting updated accordingly. Total Recording Before Delete :-"
								+ totalRecordings + "Recordings after Delete :-" + totalRecordingID.getText());
				reports.attachScreenshot(captureCurrentScreenshot());
			} else {
				FailTestCase("Recording is not deleted. Total Recording Before Delete :-" + totalRecordings
						+ "Recordings after Delete :-" + totalRecordingID.getText());
			}
		} else {
	...
	/**
	 * This function is used to delete all Series Recordings from Planned
	 * recordings Created By Rahul Dhoundiyal
	 */
	public void deletePlannedSeriesRecording() throws InterruptedException {
		...
		for (int i = 0; i < totalRecordings; i++) {
			...
			if (focusRecordingElement.getAttribute("assetvolume").equalsIgnoreCase("SERIES")) {
				...
				reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
				driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
				List<WebElement> recordingContentElementList = driver
						.findElements(By.cssSelector(ObjectRepository.RecordingElements.RecordingListCSSSelector));
	...
	/**
	 * This function is used to verify all recordings getting deleted from
	 * Planned recordings Created By Rahul Dhoundiyal
	 */
	...
	/**
	 * This function is used to schedule recordings on future channel from EPG
	 * Screen. Created By Rahul Dhoundiyal
	 */
	...
	public EpisodeInfo scheduleRecordingFromEPGScreen(String recordingType) throws InterruptedException {
		...
		while (!stopRecording) {
			...
			driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
			System.out.println(focusProgram
					.findElements(By.cssSelector(
	 ...
	 * Created By Rahul Dhoundiyal
	 */
	public boolean verifyOnGoingRecording(EpisodeInfo episodeDetails, String recordingType)
			throws InterruptedException {
		boolean verifyRecording = false;
		...
		for (int i = 0; i < recordingSize; i++) {
			...
			if ((focusRecordingElement.getAttribute("assetvolume").equalsIgnoreCase(recordingType))) {
				if (focusRecordingElement
						.findElement(By.className(ObjectRepository.RecordingElements.ChannelNoInPlannedRecording))
						.getText().equalsIgnoreCase(episodeDetails.channelNo)
						&& focusRecordingElement
								.findElement(By
										.cssSelector(ObjectRepository.RecordingElements.ProgramNameInPlannedRecording))
								.getAttribute("innerText").equalsIgnoreCase(episodeDetails.programName)) {
					if (!focusRecordingElement.findElements(By.cssSelector(".videoQuality .ongoing_recording img"))
							.isEmpty()) {
						verifyRecording = true;
						break;
					}
	...
	/**
	 * This function is used to delete planned recordings Created By Rahul
	 * Dhoundiyal
	 */
	public void stopAndVerifyOnGoingRecording() throws InterruptedException {
		...
		for (int i = 0; i < 15; i++) {
			reports.attachScreenshot(captureCurrentScreenshot());
			System.out.println(focusRecordingElement
					.findElements(By.cssSelector(ObjectRepository.RecordingElements.ongoingRecordingIconElement))
					.size());
			if (focusRecordingElement
					.findElements(By.cssSelector(ObjectRepository.RecordingElements.ongoingRecordingIconElement))
					.size() > 0) {
	...
	/**
	 * This function is used to delete ongoing and complete recordings Created
	 * By Rahul Dhoundiyal
	 */
	public void deleteAndVerifyRecordedItem() throws InterruptedException {
		...
		if (recordedItemDeleted) {
			...
			if (recordingBeforeDelete.equalsIgnoreCase(totalRecordingID.getText())) {
				FailTestCase("Recording not Delete. Recording List not getting updated Recordings Before Delete :- "
						+ recordingBeforeDelete + " Recordings After Delete :- " + totalRecordingID.getText());
			} else {
				reports.log(LogStatus.PASS, "Recorded Item getiing deleted Recordings Before Delete :- "
						+ recordingBeforeDelete + " Recordings After Delete :- " + totalRecordingID.getText());
				reports.attachScreenshot(captureCurrentScreenshot());
			}
	...
	/**
	 * This function is used to delete single recording. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to delete series recording. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to delete series recording one by one Created By
	 * Rahul Dhoundiyal
	 */
	...
	public void verifyAndScheduleRecordingForFutureEpisode() throws InterruptedException {
		...
		for (EpisodeInfo epsiodeInfo : listOfAddedRecordings) {
			...
			if (verifyMultipleSingleRecordings) {
				reports.log(LogStatus.PASS,
						"Expected Output - Recording should be scheduled for " + epsiodeInfo.programName
								+ " Actual - Recording getting scheduled for " + epsiodeInfo.programName);
				reports.attachScreenshot(captureCurrentScreenshot());
			} else {
				FailTestCase("Expected Output - Recording should be scheduled for " + epsiodeInfo.programName
						+ " Actual - Recording not getting scheduled for " + epsiodeInfo.programName);
			}
		}
	}
	...
	public void verifyAndStartRecordingForCurrentEpisode() throws InterruptedException {
		...
		if (verifyOnGoingRecording) {
			reports.log(LogStatus.PASS,
					"Expected Output - Recording should be Started for " + episodeDetails.programName
							+ " Actual - Recording getting started for " + episodeDetails.programName);
			reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
		} else {
			FailTestCase("Expected Output - Recording should be started for " + episodeDetails.programName
					+ " Actual - Recording not getting started for " + episodeDetails.programName);
		}
	}

	/**
	 * This function is used to navigate to Menu screen. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to navigate to Shop screen. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to navigate to settings screen. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to navigate to tv guide screen. Created By Rahul
	 * Dhoundiyal
	 */
	...
	/**
	 * This function is used to start recordings on current episode from DTV
	 * Screen Created By Rahul Dhoundiyal
	 */
	public List<EpisodeInfo> startRecordingFromDTV(String recordingType, int numberOfRecording)
		...
		while (numberOfRecording != noOfRecordedChannel) {
			...
			if (recordingType.equalsIgnoreCase("SINGLE")) {
				...
				if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
					programDetails.add(
							new EpisodeInfo(getChannelNo(), episodeName, getEpisodeDuration(), getChannelDefiniton()));
					...
					sendKeyMultipleTimes("DOWN", 1, 2000);
					if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
						programDetails.add(new EpisodeInfo(getChannelNo(), episodeName, getEpisodeDuration(),
								getChannelDefiniton()));
						reports.log(LogStatus.PASS,
								" Click on opnemen to start recording on - " + getInfoEpisodeName());
						sendKeyMultipleTimes("ENTER", 1, 1000);
						reports.attachScreenshot(captureCurrentScreenshot());
						checknext = true;
						noOfRecordedChannel += 1;
						...
						reports.attachScreenshot(captureCurrentScreenshot());
						sendKeyMultipleTimes("PAGE_DOWN", 1, 1000);
						// sendUnicodeMultipleTimes(Unicode.VK_PAGE_UP_OR_CHANNEL_PLUS.toString(),
						// 1, 1000);
				...
				programDetails
						.add(new EpisodeInfo(getChannelNo(), episodeName, getEpisodeDuration(), getChannelDefiniton()));
				sendKeyMultipleTimes("DOWN", 1, 1000);
				sendKeyMultipleTimes("ENTER", 1, 1000);
				driver.switchTo().defaultContent();
				wait.until(ExpectedConditions.presenceOfElementLocated(By.xpath("//p[@id='headerTitle']")));
	...
	public void verifyActionMenuRecordinStop() throws InterruptedException, IOException {
		...
		Thread.sleep(1000);
		reports.log(LogStatus.PASS,
				"Navigate to Channel with ongoing recording : " + listOfAddedRecordings.get(0).channelNo);
	...
	private List<EpisodeInfo> scheduleOverlappingRecording(int numberOfRecording)
		...
		while (numberOfRecording != recordedChannel) {
			...
			driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
			System.out.println(focusProgram
					.findElements(By.cssSelector(ObjectRepository.RecordingElements.futureRecordingIconElement))
					.isEmpty());
			if (focusProgram.findElements(By.cssSelector(ObjectRepository.RecordingElements.futureRecordingIconElement))
					.isEmpty()) {
				if (checkOverLap) {
					driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
					if (!isOverlapping(prevStartTime, prevEndTime,
							sdf.parse(epgScreen.focusElementProgramTime.getText().split(" ")[0].trim()),
							sdf.parse(epgScreen.focusElementProgramTime.getText().split(" ")[2].trim()))) {
						if (prevEndTime
								.before(sdf.parse(epgScreen.focusElementProgramTime.getText().split(" ")[2].trim()))
								|| prevEndTime.before(
										sdf.parse(epgScreen.focusElementProgramTime.getText().split(" ")[0].trim()))) {
							reports.log(LogStatus.PASS,
									"Overlapping Episode Not Found Prev Episode Timing - " + prevEpisodeDuration
											+ "Current Episode Timing :-"
						...
						reports.log(LogStatus.PASS,
								"Overlapping Episode Found Prev Episode Timing - " + prevEpisodeDuration
										+ "Current Episode Timing :-" + epgScreen.focusElementProgramTime.getText());
					}
				}
				...
				if (recordingType.equalsIgnoreCase("SINGLE")) {
					if (activeInfoMenuItem.getText().equalsIgnoreCase("opnemen")) {
						episodeDetails.add(new EpisodeInfo(getChannelNo(), epgEpisodeName, getEpisodeDuration(),
								getChannelDefiniton()));
						prevEpisodeDuration = getEpisodeDuration();
						TestInitization.sendKeyMultipleTimes("ENTER", 1, 1000);
						reports.log(LogStatus.PASS,
								"Click on opnemen to start recording - Episode TIme - " + prevEpisodeDuration);
						reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
						driver.switchTo().frame(TestInitization.getCurrentFrameIndex());
						recordedChannel += 1;
						checkOverLap = true;
					...
					TestInitization.sendKeyMultipleTimes("DOWN", 1, 1000);
					if (activeInfoMenuItem.getText().equalsIgnoreCase("serie opnemen")) {
						episodeDetails.add(new EpisodeInfo(getChannelNo(), getInfoEpisodeName(), getEpisodeDuration(),
								getChannelDefiniton()));
						TestInitization.sendKeyMultipleTimes("ENTER", 1, 1000);
						reports.log(LogStatus.PASS, "Click on serie opnemen to start recording");
						reports.attachScreenshot(TestInitization.captureCurrentScreenshot());
